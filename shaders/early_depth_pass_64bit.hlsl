...
{      
    ...
    {
        const float TriangleZ = Point0.z * BC.x + Point1.z * BC.y + Point2.z * BC.z;
        uint64_t value = asuint(TriangleZ);
        value = value << 32 | index;
        InterlockedMin(DepthBuffer[tid.xy], value); //Now Depth Buffer became Visualization Buffer
    }
}